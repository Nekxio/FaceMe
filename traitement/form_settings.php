		...
		if ($name_check == true) {
			...
			if ($pwd_check == true) {
				...
				if ($email_check == true) {
					...
					$sql="UPDATE user set name=?, mdp=PASSWORD(?), email=?, avatar=?, background=?, bio=? WHERE id=?";
					$q = $pdo->prepare($sql);
					$q->execute(array($_POST['name'],$_POST['PASSWORD'],$_POST['email'], $target_file_avatar, $target_file_background, $_POST['bio'], $_SESSION['id']));
